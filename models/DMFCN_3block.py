...
def calc_params(layers):
    total = 0
    for group in layers:
        try: 
            group_total = sum((in_ch * kernel * out_ch + out_ch) 
                         for in_ch, out_ch, kernel, _ in group)
        except:
            group_total = sum((in_ch * kernel * out_ch + out_ch) 
                         for in_ch, out_ch, kernel in group)
        print(f"Group params: {group_total:,}")
        total += group_total
    print(f"Total params: {total:,}")

# Layer configs
layers = [
    [(32, 32, 3, 1), (32, 32, 8, 1), (32, 32, 8, 5), (32, 32, 8, 10)],
    [(128, 64, 3, 1), (128, 64, 8, 1), (128, 64, 8, 5), (128, 64, 8, 10)],
    [(256, 8, 1, 1), (256, 8, 1, 1)]
]
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'imputation' or self.task_name == 'anomaly_detection':
            self.projection = nn.Linear(
               (configs.d_model//4)*4, configs.c_out, bias=True)
